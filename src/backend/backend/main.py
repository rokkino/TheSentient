...
# main.py is in src/backend/backend/main.py
# PROJECT_ROOT should be the parent of 'src', which is 3 levels up from here
PROJECT_ROOT = str(BASE_DIR.parents[2])
...
print(f"DEBUG: sys.path includes PROJECT_ROOT: {PROJECT_ROOT in sys.path}")
...
from services.scheduler_jobs import execute_orders_job, analyze_earnings_job, refresh_earnings_cache_job, cleanup_old_decisions_job, update_bot_performance_job
...
async def lifespan(app: FastAPI):
    ...
    scheduler_service.add_job(cleanup_old_decisions_job, 'interval', days=1, id='cleanup_old_decisions')
    scheduler_service.add_job(update_bot_performance_job, 'interval', minutes=30, id='update_bot_performance')
    print("Scheduled execute_orders_job, analyze_earnings_job, refresh_earnings_cache_job, cleanup_old_decisions_job, update_bot_performance_job")
    yield
    # Shutdown
    print("Shutting down...")
...
async def forgot_password(request_data: ForgotPasswordRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Simulate sending email by printing the link to the terminal
...
async def reset_password(request_data: ResetPasswordRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Convert to naive or ensure timezone awareness matches
