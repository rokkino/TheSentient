...
class Bot(Base):
    ...
    performance_history = Column(Text, nullable=True) # JSON list of {time: timestamp, value: P&L}
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            'performance_history': json.loads(self.performance_history) if self.performance_history else None,
        }
...
class Decision(Base):
    ...
    stop_loss = Column(Float, nullable=True) # Stop loss price (optional)
    take_profit = Column(Float, nullable=True) # Take profit price (optional)
    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))
