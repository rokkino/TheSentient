...
# Use absolute path to ensure consistent database location regardless of cwd
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
default_db_path = os.path.join(BASE_DIR, "thesentient.db")
        ...
        if DATABASE_URL.startswith('sqlite'):
            try:
                ...
                if 'decisions' in table_names:
                    ...
                    with engine.begin() as conn:
                        for col_name, col_type in new_decision_columns.items():
                            if col_name not in existing_columns:
                                try:
                                    conn.execute(text(f"ALTER TABLE decisions ADD COLUMN {col_name} {col_type}"))
                                    print(f"Added column {col_name} to decisions table")
                                except Exception as e:
                                    print(f"Could not add column {col_name}: {e}")
            except Exception as e:
