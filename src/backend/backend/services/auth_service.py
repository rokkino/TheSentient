...
def authenticate_user(db: Session, identifier: str, password: str) -> Optional[User]:
    ...
    if not user:
        print(f"[AUTH] User not found by username: {identifier}")
        user = get_user_by_email(db, identifier)
    if not user:
        print(f"[AUTH] User not found by email either: {identifier}")
        return None
    ...
    if not user.is_active:
        print(f"[AUTH] User is inactive: {user.username}")
        return None
        
    print(f"[AUTH] Authentication success for {user.username}")
    return user
