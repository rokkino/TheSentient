...
class EarningsService:
    ...
    def _get_cached_earnings_for_date(self, target_date: date) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            # If timestamp exists, check age (7 days max)
            if 'timestamp' in cache_data:
                cache_age = time.time() - cache_data['timestamp']
                max_cache_age = 7 * 24 * 60 * 60  # 7 days - more frequent updates
            ...
            if os.path.exists(cache_file):
                ...
                if 'data' not in cache_data:
                    ...
                    if 'timestamp' in cache_data:
                        ...
                        if cache_age <= max_cache_age:
                            print(f"[EarningsService] Using calendar cache (age: {cache_age/(24*3600):.1f} days)")
                            return cache_data['data']
                    else:
                        return cache_data['data']
                ...
                for filename in os.listdir(self.cache_dir):
                    if filename.startswith('calendar_') and filename.endswith('.json'):
                        ...
                        try:
                            ...
                            # Check timestamp if it exists (7 days max)
                            if 'timestamp' in alt_cache_data:
                                cache_age = time.time() - alt_cache_data['timestamp']
                                max_cache_age = 7 * 24 * 60 * 60  # 7 days
                                if cache_age > max_cache_age:
                                    print(f"[EarningsService] Alternative calendar cache too old (age: {cache_age/(24*3600):.1f} days), skipping")
                                    continue
